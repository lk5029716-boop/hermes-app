...
ENV_VARS = [
    ...
    # New providers (2026.5.x+) — all plain API-key auth
    ("MOONSHOT_API_KEY",         "Moonshot (Kimi)",          "provider",  True),
    ("TOGETHER_API_KEY",         "Together AI",              "provider",  True),
    ("REPLICATE_API_KEY",        "Replicate",                "provider",  True),
    ("DEEPINFRA_API_KEY",        "DeepInfra",                "provider",  True),
    ("HYPERBOLIC_API_KEY",       "Hyperbolic",               "provider",  True),
    ("CEREBRAS_API_KEY",         "Cerebras",                 "provider",  True),
    ("GROQ_API_KEY",             "Groq",                     "provider",  True),
    ("MISTRAL_API_KEY",          "Mistral AI",               "provider",  True),
    ("COHERE_API_KEY",           "Cohere",                   "provider",  True),
    ("OPENAI_API_KEY",           "OpenAI",                   "provider",  True),
    ("ANTHROPIC_API_KEY",        "Anthropic Claude",         "provider",  True),
    ("XAI_API_KEY",              "xAI Grok",                 "provider",  True),
    # Additional providers (2026.5.x+)
    ("MINIMAX_CN_API_KEY",       "MiniMax (China)",          "provider",  True),
    ("GMI_API_KEY",              "GMI Cloud",                "provider",  True),
    ("XIAOMI_API_KEY",           "Xiaomi MiLM",              "provider",  True),
    ("OLLAMA_API_KEY",           "Ollama Cloud",             "provider",  True),
    ("AZURE_FOUNDRY_API_KEY",    "Azure AI Foundry",         "provider",  True),
    ("LM_API_KEY",               "LM Studio",                "provider",  True),
    ("ZHIPU_API_KEY",            "Z.AI (GLM)",               "provider",  True),
    ("ALIBABA_CODING_PLAN_API_KEY", "Alibaba Coding Plan",   "provider",  True),
    ("TENCENT_TOKENHUB_API_KEY", "Tencent TokenHub",         "provider",  True),
    ("KILO_API_KEY",             "Kilo Code",               "provider",  True),
    ("OPENCODE_API_KEY",         "OpenCode",                 "provider",  True),
    ("AWS_ACCESS_KEY_ID",        "AWS Access Key (Bedrock)", "provider",  True),
    ("AWS_SECRET_ACCESS_KEY",    "AWS Secret Key (Bedrock)", "provider",  True),
    ...
    ("SERPAPI_API_KEY",          "SerpAPI (search)",         "tool",      True),
    ("BRAVE_API_KEY",            "Brave Search",             "tool",      True),
    ...
    ("MEM0_API_KEY",             "Mem0 Platform API key",    "memory",    True),
    ("MEM0_USER_ID",             "Mem0 user identifier",     "memory",    False),
]
...
def write_config_yaml(data: dict[str, str]) -> None:
    ...
    *only* from this file (see ``hermes_cli/mcp_config.py:_get_mcp_servers``), so
    ...
    # If Nous or xAI OAuth tokens exist in auth.json, actively set the
    # provider + base_url so the gateway uses the OAuth session — even on
    # container restart where the existing config.yaml might still say "auto".
    _auth_path = config_path.parent / "auth.json"
    _oauth_provider = None
    _oauth_base_url = None
    if _auth_path.exists():
        try:
            _auth_data = json.loads(_auth_path.read_text())
            _providers = _auth_data.get("providers", {})
            if _providers.get("nous"):
                _oauth_provider = "nous"
                _oauth_base_url = _NOUS_INFERENCE_URL
            elif _providers.get("xai-oauth"):
                _oauth_provider = "xai-oauth"
        except Exception:
            pass
    if _oauth_provider:
        merged_model["provider"] = _oauth_provider
        if _oauth_base_url:
            merged_model["base_url"] = _oauth_base_url
    ...
    # Mem0 — inject memory provider config when MEM0_API_KEY env var is set.
    # On Railway (no volumes), this ensures mem0 survives redeploys.
    # Check both .env data and os.environ (Railway env vars don't live in .env).
    mem0_key = (data.get("MEM0_API_KEY") or os.environ.get("MEM0_API_KEY", "")).strip()
    if mem0_key:
        merged_memory = dict(merged.get("memory") if isinstance(merged.get("memory"), dict) else {})
        merged_memory["provider"] = "mem0"
        merged["memory"] = merged_memory
        # Also write mem0.json so the mem0 plugin picks up the key
        mem0_json_path = Path(HERMES_HOME) / "mem0.json"
        mem0_cfg = {}
        if mem0_json_path.exists():
            try:
                mem0_cfg = json.loads(mem0_json_path.read_text())
            except Exception:
                pass
        mem0_cfg["api_key"] = mem0_key
        if uid := data.get("MEM0_USER_ID", "").strip():
            mem0_cfg["user_id"] = uid
        mem0_json_path.write_text(json.dumps(mem0_cfg, indent=2))
        try:
            os.chmod(mem0_json_path, 0o600)
        except OSError:
            pass
...
def write_env(path: Path, data: dict[str, str]) -> None:
    ...
    cat_order = ["model", "provider", "tool", "memory",
...
_xai_oauth_task: asyncio.Task | None = None  # track the poll task so we can cancel it
...
async def api_oauth_xai_delete(request: Request) -> Response:
    global _xai_oauth_state, _xai_oauth_task
    if err := guard(request):
        return err
    # Cancel active poll task
    if _xai_oauth_task is not None and not _xai_oauth_task.done():
        _xai_oauth_task.cancel()
        try:
            await _xai_oauth_task
        except (asyncio.CancelledError, Exception):
            pass
        _xai_oauth_task = None
    auth_path = Path(HERMES_HOME) / "auth.json"
    if auth_path.exists():
        try:
    ...
    # Reset config.yaml provider — otherwise gateway restarts with provider=xai-oauth
    # and immediately fails with "No access token found".
    import yaml
    config_path = Path(HERMES_HOME) / "config.yaml"
    if config_path.exists():
        try:
            ...
            if isinstance(loaded, dict) and isinstance(loaded.get("model"), dict):
                if loaded["model"].get("provider") == "xai-oauth":
                    loaded["model"]["provider"] = "auto"
                    loaded["model"].pop("default", None)
                with config_path.open("w") as f:
                    yaml.safe_dump(loaded, f, sort_keys=False, default_flow_style=False)
        except Exception:
            pass
...
async def api_oauth_xai_start(request: Request) -> Response:
    ...
    # Cancel any previous poll task before starting a new flow
    if _xai_oauth_task is not None and not _xai_oauth_task.done():
        _xai_oauth_task.cancel()
        try:
            await _xai_oauth_task
    ...
    _xai_oauth_task = asyncio.create_task(_poll_xai_device_auth(_xai_oauth_state))
...
# ── Nous Research Portal OAuth (Device Code + Agent Key Mint) ─────────────────
# Nous Portal at portal.nousresearch.com uses a standard RFC 8628 device-code
# flow, then mints a short-lived "agent key" that serves as the bearer token
# for inference-api.nousresearch.com.
#
# Flow:
#   1. POST /api/oauth/device/code  → device_code, user_code, verification_uri
#   2. Poll /api/oauth/token        → access_token (when user approves)
#   3. POST /api/oauth/agent-key    → api_key (minted from access_token)
#   4. Store in auth.json + config.yaml, restart gateway

_NOUS_PORTAL_URL      = "https://portal.nousresearch.com"
_NOUS_INFERENCE_URL   = "https://inference-api.nousresearch.com/v1"
_NOUS_CLIENT_ID       = "hermes-cli"
_NOUS_SCOPE           = "inference:mint_agent_key"
_NOUS_DEVICE_URL      = f"{_NOUS_PORTAL_URL}/api/oauth/device/code"
_NOUS_TOKEN_URL       = f"{_NOUS_PORTAL_URL}/api/oauth/token"
_NOUS_AGENT_KEY_URL   = f"{_NOUS_PORTAL_URL}/api/oauth/agent-key"
_NOUS_GRANT_TYPE      = "urn:ietf:params:oauth:grant-type:device_code"

_nous_oauth_state: dict | None = None
_nous_oauth_task: asyncio.Task | None = None  # track the poll task so we can cancel it


def _has_nous_oauth_tokens() -> bool:
    """True when auth.json contains a valid Nous agent_key or refresh token."""
    auth_path = Path(HERMES_HOME) / "auth.json"
    if not auth_path.exists():
        return False
    try:
        ...
        return bool(
            ...
            and (state.get("agent_key") or state.get("refresh_token"))
        )
    except Exception:
        return False


def _save_nous_auth_json(tokens: dict, model: str = "") -> None:
    """Write Nous OAuth tokens + agent key to auth.json."""
    auth_path = Path(HERMES_HOME) / "auth.json"
    existing: dict = {}
    if auth_path.exists():
        try:
    ...
    providers = existing.setdefault("providers", {})
    # Write tokens as a flat dict — the gateway's NousPortalAdapter expects
    # agent_key, access_token, refresh_token etc. at the top level of
    # providers["nous"], NOT nested inside a "tokens" sub-key.
    nous_state = dict(tokens)
    nous_state["auth_mode"] = "oauth_device"
    nous_state["last_refresh"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    nous_state["portal_base_url"] = _NOUS_PORTAL_URL
    nous_state["inference_base_url"] = _NOUS_INFERENCE_URL
...
def _apply_nous_oauth_config(model: str) -> None:
    ...
    merged_model["base_url"] = _NOUS_INFERENCE_URL
    merged["model"] = merged_model
    ...
    if model:
        existing_env = read_env(ENV_FILE)
        existing_env["LLM_MODEL"] = model
        existing_env["_MODEL_NOUS"] = model
        write_env(ENV_FILE, existing_env)


async def _mint_nous_agent_key(access_token: str, model: str = "") -> dict:
    """Mint an agent key from the Nous Portal using the OAuth access token.

    Matches the hermes-cli auth.py _mint_agent_key format exactly:
    - No explicit Content-Type header (httpx sets it automatically from json=)
    - min_ttl_seconds: 300 (5 min) to match hermes-cli default
    - No model field — the Portal rejects it with 500 if included.
      The model is scoped by the OAuth grant, not the mint request.
    """
    client = get_http_client()
    body: dict = {"min_ttl_seconds": 300}
    resp = await client.post(
        _NOUS_AGENT_KEY_URL,
    ...
    if resp.status_code != 200:
        try:
            ...
            err_desc = err_payload.get("error_description") or err_payload.get("error") or resp.text[:200]
        except Exception:
            err_desc = resp.text[:200]
        raise RuntimeError(f"Agent key mint failed: {resp.status_code}: {err_desc}")
    data = resp.json()
    if "api_key" not in data:
        raise RuntimeError("Agent key mint response missing api_key")
    return data


async def _poll_nous_device_auth(state: dict) -> None:
    """Background task: poll Nous token endpoint, then mint agent key."""
    client = get_http_client()
    while time.time() < state["expires_at"]:
        await asyncio.sleep(state["interval"])
        try:
        ...
        if resp.status_code == 200:
            ...
            if not access_token:
                state["status"] = "error"
                state["error"] = "No access_token in Nous response"
                return

            # Step 3: Mint agent key
            try:
                agent_key_data = await _mint_nous_agent_key(
                    access_token, state.get("model", "")
                )
            except Exception as e:
                state["status"] = "error"
                # e is already a RuntimeError("Agent key mint failed: ...") — use str(e) directly
                state["error"] = str(e)
                print(f"[nous-oauth] mint error: {e!r}", flush=True)
                return

            # Merge agent key into tokens and save
            tokens["agent_key"] = agent_key_data["api_key"]
            tokens["agent_key_expires_at"] = agent_key_data.get("expires_at", "")
            _save_nous_auth_json(tokens, state.get("model", ""))
...
async def api_oauth_nous_start(request: Request) -> Response:
    global _nous_oauth_state, _nous_oauth_task
    if err := guard(request):
        return err
    ...
    # Cancel any previous poll task before starting a new flow
    if _nous_oauth_task is not None and not _nous_oauth_task.done():
        _nous_oauth_task.cancel()
        try:
            await _nous_oauth_task
        except (asyncio.CancelledError, Exception):
            pass
        _nous_oauth_task = None

    client = get_http_client()
    try:
        resp = await client.post(
            _NOUS_DEVICE_URL,
            data={"client_id": _NOUS_CLIENT_ID, "scope": _NOUS_SCOPE},
            headers={"Content-Type": "application/x-www-form-urlencoded"},
            timeout=httpx.Timeout(15.0),
        )
    except Exception as e:
        return JSONResponse({"error": f"Could not reach Nous Portal: {e}"}, status_code=502)

    if resp.status_code != 200:
        # Surface the actual Nous Portal error so the user sees what went wrong
        # instead of a generic "cleaning session" / redirect page.
        try:
            err_body = resp.json()
            err_detail = err_body.get("error_description") or err_body.get("error") or resp.text[:300]
        except Exception:
            err_detail = resp.text[:300]
        print(f"[nous-oauth] device code request failed: {resp.status_code}: {err_detail}", flush=True)
        return JSONResponse(
            {"error": f"Nous Portal error ({resp.status_code}): {err_detail}"},
            status_code=502,
        )
    ...
    if missing:
        return JSONResponse(
            {"error": f"Nous response missing fields: {', '.join(missing)}"},
            status_code=502,
        )
    ...
    _nous_oauth_task = asyncio.create_task(_poll_nous_device_auth(_nous_oauth_state))
...
async def api_oauth_nous_delete(request: Request) -> Response:
    ...
    if auth_path.exists():
        try:
            ...
            pass
    env = read_env(ENV_FILE)
    env.pop("_MODEL_NOUS", None)
    write_env(ENV_FILE, env)
    _nous_oauth_state = None
    # Reset config.yaml provider — otherwise gateway restarts with provider=nous
    # and immediately fails with "No access token found".
    import yaml
    config_path = Path(HERMES_HOME) / "config.yaml"
    if config_path.exists():
        try:
            with config_path.open() as f:
                loaded = yaml.safe_load(f)
            if isinstance(loaded, dict) and isinstance(loaded.get("model"), dict):
                if loaded["model"].get("provider") == "nous":
                    loaded["model"]["provider"] = "auto"
                    loaded["model"].pop("default", None)
                    loaded["model"].pop("base_url", None)
                with config_path.open("w") as f:
                    yaml.safe_dump(loaded, f, sort_keys=False, default_flow_style=False)
        except Exception:
            pass
...
# ── WebSocket auth token (query-param fallback) ──────────────────────────────
# Browsers / proxies do not always forward cookies with WebSocket upgrade
# requests (SameSite, CDN stripping, Safari).  We sidestep this by injecting a
# short-lived HMAC-signed token into the HTML page and having the frontend JS
# pass it as `?token=<value>` in the WebSocket URL.
WS_TOKEN_TTL = 300  # 5 minutes — the SPA may sit open while users configure before opening the terminal

def _make_ws_token() -> str:
    """Generate a short-lived token for WebSocket auth."""
    expires = str(int(time.time()) + WS_TOKEN_TTL)
    sig = _hmac.new(COOKIE_SECRET, f"ws:{expires}".encode(), _hashlib.sha256).hexdigest()
    return f"{expires}.{sig}"


def _verify_ws_token(token: str) -> bool:
    """Verify a WebSocket auth token from query param."""
    try:
        expires_s, sig = token.rsplit(".", 1)
        if int(expires_s) < time.time():
            return False
        expected = _hmac.new(COOKIE_SECRET, f"ws:{expires_s}".encode(), _hashlib.sha256).hexdigest()
        return _hmac.compare_digest(sig, expected)
    except Exception:
        return False
...
def _is_ws_authenticated(websocket: WebSocket) -> bool:
    """Check cookie first, fall back to ?token= query param for WebSocket."""
    if _is_authenticated(websocket):
        return True
...
async def page_index(request: Request):
    ...
    ws_token = _make_ws_token()
    return templates.TemplateResponse(request, "index.html", {"ws_token": ws_token})
...
async def api_config_reset(request: Request) -> Response:
    if err := guard(request): return err
    # Cancel any active OAuth poll tasks
    global _xai_oauth_task, _nous_oauth_task, _xai_oauth_state, _nous_oauth_state
    for task in (_xai_oauth_task, _nous_oauth_task):
    ...
    async with cfg_lock:
        if ENV_FILE.exists():
            ENV_FILE.unlink()
        # Also clear OAuth tokens from auth.json so they don't persist across reset
        auth_path = Path(HERMES_HOME) / "auth.json"
        if auth_path.exists():
            try:
                data = json.loads(auth_path.read_text(encoding="utf-8"))
...
async def terminal_ws(websocket: WebSocket) -> None:
    """Spawn /bin/bash with a real PTY and bridge it over a WebSocket.

    Protocol — text messages from the client:
        {data}         → raw terminal input (keystrokes)
        resize:C,R     → resize terminal to C columns × R rows
        ping           → keepalive; server replies with "pong"

    Protocol — text messages to the client:
        {data}         → raw terminal output (ANSI escape sequences included)
        pong           → keepalive reply

    Binary messages are forwarded as-is (for future passthrough of binary
    protocols like SSH keys, though xterm.js sends text by default).
    """
    ...
    # ── 2. Spawn a PTY with subprocess (more reliable on container runtimes) ─
    os.environ.setdefault("TERM", "xterm-256color")
    shell = os.environ.get("SHELL", "/bin/bash")
    # Use a minimal, container-safe environment for the shell.
    # Passing the full server env (which includes secrets like API keys)
    # is unnecessary and can cause shell init scripts to misbehave.
    shell_env = {
    ...
    # Carry through SSH_AUTH_SOCK / SSH_AGENT_LAUNCHER if present (useful for git ops)
    for k in ("SSH_AUTH_SOCK", "SSH_AGENT_LAUNCHER", "GIT_EDITOR", "GIT_PAGER"):
        if k in os.environ:
            shell_env[k] = os.environ[k]
    try:
        ...
        print(f"[terminal] Failed to spawn PTY shell: {exc}", flush=True)
        try:
            await websocket.send_text(f"\r\n\x1b[31mShell spawn failed: {exc}\x1b[0m\r\n")
        except Exception:
            pass
        await websocket.close(code=1011)
        return
    ...
    # Configure master fd as non-blocking
    try:
    ...
    try:
        ...
        # ── 3. Pump: PTY → WebSocket ──────────────────────────────────────
        async def pty_to_ws():
            while True:
                try:
                    data = await loop.run_in_executor(None, os.read, fd, 8192)
                    if not data:
                        print(f"[terminal] shell (pid={pid}) exited — PTY read returned empty", flush=True)
                        break
                    ...
                    print(f"[terminal] pty_to_ws OSError errno={errno_val} pid={pid}: {exc!r}", flush=True)
                    break
                except Exception as exc:
                    print(f"[terminal] pty_to_ws unexpected error pid={pid}: {exc!r}", flush=True)
                    break

        # ── 4. Pump: WebSocket → PTY ──────────────────────────────────────
        async def ws_to_pty():
            while True:
                try:
        ...
        # ── 5. Bidirectional pump + process monitor ───────────────────────
        async def proc_watchdog():
            """Detect if the subprocess exits and close the pump."""
            while True:
                if proc.returncode is not None:
                    print(f"[terminal] shell (pid={pid}) exited with code {proc.returncode}", flush=True)
                    break
            ...
            (
                ...
                asyncio.create_task(proc_watchdog()),
            ),
            return_when=asyncio.FIRST_COMPLETED,
        )
        for t in pending:
...
def _is_eagain(exc: OSError) -> bool:
    """Check if an OSError is retryable (EAGAIN, EWOULDBLOCK, or EIO).

    EIO on a non-blocking PTY master can occur when the child process is
    still alive but has no data, or on container runtimes (Docker/Railway)
    where the PTY slave side may briefly appear closed.  Treat it as
    retryable — if the child has genuinely exited, the next read will
    also return EIO and we'll break out of the pump loop.
    """
    if not hasattr(exc, "errno"):
        return False
    return exc.errno in (errno.EAGAIN, errno.EWOULDBLOCK, errno.EIO)
...
routes = [
    ...
    # Web Terminal — spawns /bin/bash with a PTY, accessible only to admins.
    WebSocketRoute("/setup/api/terminal",               terminal_ws),
